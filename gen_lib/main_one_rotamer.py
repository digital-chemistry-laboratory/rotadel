    ...
    if output_json is not None:
        ...
        if output_json.is_dir():
            raise IsADirectoryError(
                f"Provided path {output_json} is a directory. Please provide a file path."
            )
        elif output_json.exists():
            raise FileExistsError(
                f"Provided path {output_json} already exists. "
                f"Please provide a different file name to avoid overwriting."
            )
